...
function updateRobotPartsData(controlDataControleValue, controlParentElement) {
    ...
    if (controlDataControleValue === "-" && parseInt(dataElement.value) > 0) {
        dataElement.value = parseInt(dataElement.value) - 1;
    ...
    statisticElements.forEach((statisticElement) => {
        ...
        if(controlDataControleValue === "-" && parseInt(dataElement.value) > 0) {
            statisticElement.innerText =
                parseInt(statisticElement.innerText) -
                parseInt(parts[dataPecaValue][statisticElement.dataset.estatistica]);
        } else if (controlDataControleValue === "+") {
            statisticElement.innerText =
                parseInt(statisticElement.innerText) +
                parseInt(parts[dataPecaValue][statisticElement.dataset.estatistica]);
...
controls.forEach((control) => { 
    control.addEventListener('click', (event) => {
        updateRobotPartsData(event.target.dataset.controle, event.target.parentNode);
        updateRobotStatistics(event.target.dataset.controle, event.target.parentNode);
    })
})