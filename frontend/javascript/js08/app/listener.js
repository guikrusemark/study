...
function setListeners(books) {
    btns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const filteredBooks = books.filter(book => (book.categoria === btn.value));
            if (btn.value)
                if (isBooksFiltered) {
                    renderBooks(books);
    ...
    btnDisponiveis.addEventListener('click', () => {
        if (isBooksFiltered) {
            ...
            document.getElementById('valor_total_livros_disponiveis').appendChild(divOferta);
        }
    });
    ...
    function sumBookPrices(books) {
        return parseFloat(books.map(book => book.preco).reduce((acc, curr) => acc + curr)).toFixed(2).replace('.', ',');
    }
}
