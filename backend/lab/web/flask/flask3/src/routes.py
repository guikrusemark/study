...
def init_routes(
    ...
    def update_user(user_id: int) -> tuple[Response, int]:
        ...
        try:
            ...
            if user_update.username:
                existing: UserResponse | None = user_dal.get_by_username(user_update.username)
            ...
            user: UserResponse | None = user_dal.update(user_id, user_update, UserResponse)
            if user is None:
                return jsonify({"error": "User not found"}), 404
            return jsonify(user.model_dump(mode='json')), 200
        except ValidationError as e:
    ...
    def get_persons() -> tuple[Response, int]:
        ...
        if first_name or last_name:
            persons: List[PersonResponse] = person_dal.search_by_name(first_name, last_name)
        else:
    ...
    def update_person(person_id: int) -> tuple[Response, int]:
        ...
        try:
            ...
            person: PersonResponse | None = person_dal.update(person_id, person_update, PersonResponse)
            if person is None:
                return jsonify({"error": "Person not found"}), 404
            return jsonify(person.model_dump(mode='json')), 200
        except ValidationError as e:
