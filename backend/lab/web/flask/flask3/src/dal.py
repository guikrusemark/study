...
class BaseDAL(
    ...
    def create(
        self, data: CreateSchemaType, response_model: Type[ResponseSchemaType]
    ) -> ResponseSchemaType:
        """Create a new record.
    ...
    def get_by_id(
        self, id: int, response_model: Type[ResponseSchemaType]
        ...
        cached: Optional[ResponseSchemaType] = self.cache.get(
            self._get_cache_key(id), response_model
        )
    ...
    def get_all(
        self, response_model: Type[ResponseSchemaType]
        ...
        cached: Optional[List[ResponseSchemaType]] = self.cache.get_list(
            self._get_list_cache_key(), response_model
        )
        if cached is not None:
            return cached
    ...
    def update(
        ...
        update_data: dict[str, Optional[str | int]] = data.model_dump(
            exclude_unset=True
        )
