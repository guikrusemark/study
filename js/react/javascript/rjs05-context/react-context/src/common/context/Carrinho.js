  ...
  return (
    <CarrinhoContext.Provider
      value={ {
        ...
        quantidadeTotal, setQuantidadeTotal,
        total, setTotal
      } }>
      { children }
    </CarrinhoContext.Provider>
...
export const useCarrinhoContext = (id) => {
  ...
  const [ quantidade, setQuantidade ] = useState(carrinho.find(item => item.id === id)?.quantidade || 0);
  ...
  function toggleProduto(sum, produto) {
    if (quantidade > 0 | sum > 0) {
      ...
      if (index === -1) // if doesn't exist on carrinho, then push it
        carrinho.push({ ...produto });
      else
        carrinho[ index ] = { ...produto }; // if it exists, replace it
